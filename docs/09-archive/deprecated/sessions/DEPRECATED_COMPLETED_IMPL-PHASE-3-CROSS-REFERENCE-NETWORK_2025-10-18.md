...
graph TD
    ...
    A <--> C[Database-Theme-System Implementation]
    B <--> D[Theme Service Layer]
    C <--> E[Migration 027 Schema]
    D <--> F[PDF Integration Examples]
    E <--> G[Critical Fixes Protection]
    
    H[Planning Documents] --> A
    H --> B
    I[Session Reports] --> C
    I --> D
    J[Quality Standards] --> B
    J --> G
```
