...
> **Erstellt:** 15.10.2025 | **Letzte Aktualisierung:** 25.10.2025 (PATHS System Korrektur - Main/Renderer Process Trennung präzisiert)  
...
3. **MANDATORY:** Heutiges Datum verwenden: 23.10.2025
...
## 🗄️ DATABASE-CHAOS RESOLUTION (22.10.2025)

**PROBLEM SOLVED:** Multiple verwaiste Datenbanken verursachten falsche Debug-Analysen
...
- `/db/rawalite.db` (0KB - war leer!) → Entfernt
- `/db/after-migration-040*.db` → Archiviert in `/db/archive-migration-backups/`
- `/db/real-rawalite.db` → Archiviert in `/db/archive-migration-backups/`

**FÜR DB-ANALYSE VERWENDEN:**
```bash
node scripts/ANALYZE_DATABASE_SQLJS_INSPECT.mjs  # Nutzt korrekte DB
```

**VOLLSTÄNDIGE DOKUMENTATION:** [LESSON_FIX-DATABASE-MULTIPLE-INSTANCES-CHAOS_2025-10-22.md](docs/06-lessons/sessions/LESSON_FIX-DATABASE-MULTIPLE-INSTANCES-CHAOS_2025-10-22.md)
...
- **Paths**:  
  - Renderer Process: Nur über `src/lib/paths.ts` (`PATHS`).  
  - Main Process: Darf `app.getPath()` verwenden (native Electron APIs).  
  - IPC Bridge: `electron/ipc/paths.ts` für Renderer-Main communication.  
  - Standalone: `src/lib/path-utils.ts`.  
  - **Verboten:** direkter `app.getPath()` im **Renderer Process**.  
- **Persistenz**:  
...
## 🚫 Verbotene Muster (niemals generieren) - ERWEITERT 25.10.2025

- `npm` oder `yarn` Befehle.  
- `app.getPath()` im **Renderer Process** (außerhalb `paths.ts` und `electron/ipc/paths.ts`).  
...
- **🚨 NEW (25.10.2025):** PATHS System Regeln missachten (Main Process darf `app.getPath()`, Renderer nicht)
