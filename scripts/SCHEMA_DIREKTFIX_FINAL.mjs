 ...
 * Backward compatible fix without migration system disruption
 */

async function applySchemaFixes() {
  console.log('🛠️  SCHEMA DIREKTFIX - Navigation Grid Settings & Theme Schema');
  ...
  try {
    ...
    const nullGridCheck = db.exec(`
      SELECT mode_name, user_id, grid_template_areas, grid_template_columns, grid_template_rows
    ...
    if (nullGridModes.length > 0) {
      ...
      // Grid Configuration (from DatabaseNavigationService.ts)
    ...
    console.log(`   display_name column: ${hasDisplayName ? '✅ EXISTS' : '❌ MISSING'}`);
    
    if (!hasDisplayName) {
      try {
        db.exec('ALTER TABLE themes ADD COLUMN display_name TEXT');
        console.log('   ✅ display_name column added');
        
        // Update existing themes with display_name = name
        db.exec('UPDATE themes SET display_name = name WHERE display_name IS NULL');
        console.log('   ✅ display_name populated for existing themes');
        
      } catch (error) {
        if (error.message.includes('duplicate column name')) {
          console.log('   ℹ️  display_name column already exists');
        } else {
          console.log(`   ❌ Failed to add display_name: ${error.message}`);
          throw error;
        }
      }
    }
    ...
    // Check theme schema
    const finalThemeCheck = verifyDb.exec('PRAGMA table_info(themes)');
    const finalColumns = finalThemeCheck[0]?.values.map(row => row[1]) || [];
    const finalHasDisplayName = finalColumns.includes('display_name');
    
    console.log(`   ✅ Navigation modes with NULL settings: ${finalNullCount}`);
    console.log(`   ✅ Theme display_name column: ${finalHasDisplayName ? 'EXISTS' : 'MISSING'}`);
    ...
    const sampleNavSettings = verifyDb.exec(`
      SELECT mode_name, grid_template_areas, grid_template_columns, grid_template_rows 
      FROM user_navigation_mode_settings 
      ORDER BY mode_name
      LIMIT 3
    `);
